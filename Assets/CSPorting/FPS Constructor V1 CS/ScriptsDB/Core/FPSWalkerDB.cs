...
public class FPSWalkerDB : MonoBehaviour
{
    ...
    {
        speed = crouchSpeed;
        GetComponent<CharacterController>().height -= crouchDeltaHeight;
        GetComponent<CharacterController>().center -= new Vector3(0, crouchDeltaHeight / 2, 0);
        crouching = true;
    }
    ...
    public void Sprinting()
    {
        ...
        {
            crouching = false;
            GetComponent<BoxCollider>().size += new Vector3(0, crouchDeltaHeight, 0);
            GetComponent<BoxCollider>().center += new Vector3(0, crouchDeltaHeight, 0);
        }
        if (canSprint)
        {
            speed = sprintSpeed;
