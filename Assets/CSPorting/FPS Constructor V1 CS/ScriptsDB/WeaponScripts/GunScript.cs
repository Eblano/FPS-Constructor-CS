...
public class GunScript : MonoBehaviour
{
	...
	*/
	...
	void Awake()
	{
		...
		ammo = GetComponent<AmmoDisplay>();
		sprint = aim1.GetComponent<SprintDisplay>();
		wepDis = GetComponent<WeaponDisplay>();
		ammo.enabled = false;
	...
	void Start()
	{
		...
		{
			gunActive = false;
			GunScript[] wpns = GetComponents<GunScript>();
	...
	public void FireOneProjectile()
	{
		...
		instantiatedProjectile = UnityEngine.Object.Instantiate(projectile, launchPos.position, convert);
		...
		Physics.IgnoreCollision(instantiatedProjectile.GetComponent<Collider>(), transform.root.GetComponentInChildren<Collider>());
		Physics.IgnoreCollision(instantiatedProjectile.GetComponent<Collider>(), player.transform.GetComponentInChildren<Collider>());
	...
	public void FireOneBullet()
	{
		...
		{
			ParticleSystem emitter = tracer.GetComponent<ParticleSystem>();
			shotCountTracer = 0;
			/*if(hits.length > 0){
				if(Vector3.Distance(hits[0].point, transform.position) >= minDistForTracer){
					...
					emitter.GetComponent<ParticleSystem>().Simulate(simulateTime);
				}
			}else{*/
